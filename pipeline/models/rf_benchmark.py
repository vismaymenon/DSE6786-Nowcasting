...
Usage (import):
    from rf_benchmark import fit_rf_nowcast
    results = fit_rf_nowcast(gdp_series)

Usage (standalone):
    python3 rf_benchmark.py
"""
...
def rf_model_nowcast(X, y, n_trees=N_TREES_FINAL, max_samples=MAX_SAMPLES, random_state=RANDOM_STATE):
    """
    POOS-compatible RF wrapper. Treats the last row of X/y as the test observation.
    Uses fixed hyperparameters (no CV tuning) for speed inside the POOS loop.
    """
    X_train = X.iloc[:-1]
    ...
    y_test_predicted  = float(rf.predict(X_test)[0])
...
if __name__ == "__main__":
    ...
    from pipeline.load_data import load_gdp
